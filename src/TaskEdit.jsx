    ...
    return (
        <div className="bg-[var(--bg)]">
            ...
            <div className="flex justify-center gap-10 items-start my-10">
                ...
                <div className="w-[35%] rounded-2xl bg-[var(--cardbg)] border-2 border-[var(--anyborder)] shadow-[0_0_35px_color-mix(in_srgb,var(--homeshadow)_25%,transparent)] relative overflow-hidden px-7">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="pt-5 pb-3.5 border-b-[1px] border-[var(--anyborder)]">
                            <h2 className="flex text-[var(--tasktext)] text-xl font-semibold gap-x-2 items-center uppercase tracking-[0.05em]">
                                <FaEdit size={30} color="var(--text)" className="mb-1.5" /> Edit Task
                            </h2>
                        </div>

                        {/*Basic Information*/}
                        <h2 className="flex text-[var(--tasktext)] font-semibold gap-x-2 py-3 mt-2"><FaCircleInfo size={17} color="var(--text)" /> Basic Information</h2>
                        ...
                        <div className="flex text-[var(--tasktext)] justify-center gap-[3%]">
                            <div className="flex flex-col w-[50%]">
                                ...
                                <input type="text" defaultValue={task.taskname} onChange={(e) => SetTask(prevtask => ({ ...prevtask, taskname: e.target.value }))} placeholder="Enter task title..." className="w-full px-4 py-2 bg-transparent border border-[var(--textfieldboarder)] rounded-md focus:outline-0 focus:border-[var(--text)] focus:shadow-[0_0_4px_color-mix(in_srgb,var(--subtext)_67%,transparent)]" />
                            </div>
                            <div className="flex flex-col w-[50%]">
                                <label className="text-[var(--subtext)] text-sm mb-2 mt-2">Category</label>
                                <select value={task.taskcategory} onChange={(e) => SetTask(prevtask => ({ ...prevtask, taskcategory: e.target.value }))} className="w-full bg-[var(--cardbg)] px-4 py-2.5 border border-[var(--textfieldboarder)] rounded-md focus:outline-0 focus:border-[var(--text)] focus:shadow-[0_0_4px_color-mix(in_srgb,var(--subtext)_67%,transparent)]">
                        ...
                        <div className="w-full flex flex-col pb-5">
                            ...
                            <textarea defaultValue={task.taskdescription} onChange={(e) => SetTask(prevtask => ({ ...prevtask, taskdescription: e.target.value }))} placeholder="Enter detailed task description..." className="text-[var(--tasktext)] bg-transparent px-4 py-2 border border-[var(--textfieldboarder)] rounded-md focus:outline-0 focus:border-[var(--text)] focus:shadow-[0_0_4px_color-mix(in_srgb,var(--subtext)_67%,transparent)] max-h-36 min-h-36" />
                        </div>

                        {/* Scenarios*/}
                        <div className="flex flex-col mb-8">
                            <div className="flex justify-between items-center">
                                <h2 className="flex text-[var(--tasktext)] font-semibold gap-x-2 py-3"><FaLightbulb size={17} color="var(--text)" /> Scenarios</h2>
                            ...
                            {cards.length > 0 &&
                                <button className={`flex gap-x-1 mt-2 py-2 w-full border border-dashed ${cards.length < 5 ? "text-[var(--text)] border-[var(--text)] bg-transparent" : "text-gray-500 border-gray-600 bg-[var(--anyborder)]"} justify-center rounded-md text-sm`} onClick={() => { cards.length < 5 ? addsncard() : null }}>
                                    <Plus size={15} color={cards.length < 5 ? "var(--text)" : "#b3b3b2"} />
                        ...
                        {/*Additional settings*/}
                        <h2 className="flex text-[var(--tasktext)] font-semibold gap-x-2 py-3"><IoMdSettings size={20} color="var(--text)" /> Additional Settings</h2>
                        ...
                        <div className="flex text-[var(--tasktext)] justify-around pb-10 gap-[3%]">
                            <div className="flex flex-col w-[50%]">
                                ...
                                <input type="number" defaultValue={task.tasktime} placeholder="e.g., 30" onChange={(e) => SetTask(prevtask => ({ ...prevtask, tasktime: parseInt(e.target.value, 10) }))} max={120} min={30} className="px-4 py-2 bg-transparent border border-[var(--textfieldboarder)] rounded-md focus:outline-0 focus:border-[var(--text)] focus:shadow-[0_0_4px_color-mix(in_srgb,var(--subtext)_67%,transparent)]" />
                            </div>
                            <div className="flex flex-col w-[50%]">
                                <label className="text-[var(--subtext)] text-sm mb-2 mt-2">Maximum Points</label>
                                <input type="number" defaultValue={task.taskpoints} onChange={(e) => SetTask(prevtask => ({ ...prevtask, taskpoints: parseInt(e.target.value, 10) }))} placeholder="e.g., 100" max={200} min={20} className="px-4 py-2 bg-transparent border border-[var(--textfieldboarder)] rounded-md focus:outline-0 focus:border-[var(--text)] focus:shadow-[0_0_4px_color-mix(in_srgb,var(--subtext)_67%,transparent)]" />
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="w-[95%] break-words mx-auto my-3">
                                    <h2 className="text-[var(--tasktext)] text-2xl font-bold text-center">
                                        {task.taskname === "" ? <span className="text-[var(--subtext)]">Ex. Task Name</span> : task.taskname}
                                    </h2>
                                </div>
